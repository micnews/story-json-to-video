...
module.exports = async (json, opts = {}) => {
  ...
  for (const page of json.pages) {
    ...
    console.log(`${consolePrefix} Preparing background media content`);
    ...
    switch (fillElement.type) {
      case 'image':
        if (fillElement.source.endsWith('.gif')) {
          file = await prepareContent.video(opts, fillElement.source);
        } else {
          file = await prepareContent.image(opts, fillElement.source, 3);
        }
        break;
      case 'video':
        file = await prepareContent.video(opts, fillElement.sources[0].source);
        break;
      default:
